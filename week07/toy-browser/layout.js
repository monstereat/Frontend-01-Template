...
function layout(element){
  ...
  if(mainSpace < 0){
    // overflow (happens only if container is single line), scale every items
    ...
    flexLine.forEach(function (items){
      ...
      if(flexTotal > 0){
        ...
        // There is *NO* flexible items which means, jusfifyContent shoud work
        if(style.jusfifyContent === 'flex-start'){
          var currentMain = mainBase
          var step = 0
        }
        if(style.jusfifyContent === 'flex-end'){
        ...
        if(style.jusfifyContent === 'center'){
          var currentMain = mainSpace / 2 * mainSign + mainBase
          var step = 0
        }
        if(style.jusfifyContent === 'space-between'){
          var currentMain = mainSpace / (items.length - 1) * mainSign
          var currentMain = mainBase
        }
        if(style.jusfifyContent === 'space-around'){
